...
class NeuralNetClassifier(GeneralClassifier):

    def __init__(self, regularizer=0.0, learning_rate=0.005, batchsize=64, epochs=50, patience=10, verbose=0,
                architecture=[32,64,128], hidden_activation_function='relu', output_activation_function='sigmoid'):
        """
        Parameters
        -------
    ...
    def optimize_hyperparameters(self, X_train, y_train, regularizers=[0, 0.01, 0.1, 1], learning_rates=[0.001, 0.01, 0.1]): 
        ...
        df_grid_search = pd.DataFrame(columns=['regularizer', 'learning_rate', 'acc'])
